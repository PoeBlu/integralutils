...
class SpenderCuckooParser(BaseSandboxParser):          
    def __init__(self, config, json_report_path, screenshot=True, whitelister=None):
        ...
        if not self.load_from_cache():
            self.logger.info("Parsing Spender Cuckoo sandbox report: " + json_report_path)
            ...
            if not self.md5:
                self.logger.critical("Unable to parse Spender Cuckoo MD5 from: " + str(json_report_path))
                return None
